...
export const memoryAidRules: MemoryAidRule[] = [
  ...
  // ========== 管理机构类（优先级90） ==========
  {
    questionPattern: /管理机构|管理部门|负责|制定|许可/,
    priority: 90,
    generateAid: (question, correctAnswers) => {
  ...
  // ========== 操作规范类（优先级75） ==========
  {
    questionPattern: /呼叫|CQ|通联|守听|发起/,
    priority: 75,
    generateAid: (question, correctAnswers) => {
  ...
  // ========== 禁止行为类（优先级80） ==========
  {
    questionPattern: /禁止|不得|严禁|不可以|错误/,
    priority: 80,
    generateAid: (question, correctAnswers) => {
  ...
  // ========== 频率划分类（优先级80） ==========
  {
    questionPattern: /频率划分|主要业务|次要业务|划分|分配|指配/,
    priority: 80,
    generateAid: (question, correctAnswers) => {
  ...
  // ========== 业余业务定义类（优先级75） ==========
  {
    questionPattern: /业余业务|卫星业余|定义|自我训练|相互通信/,
    priority: 75,
    generateAid: (question, correctAnswers) => {
  ...
  // ========== 默认规则（优先级1，兜底） ==========
  {
    ...
    generateAid: (question, correctAnswers) => {
      if (correctAnswers.length === 1) {
        ...
        // 多选题
        return `${correctAnswers.length}个正确选项`
      }
    }
  }
]
